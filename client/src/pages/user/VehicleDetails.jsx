...
const VehicleDetails = () => {
  ...
  const { pickupDate, dropoffDate } = useSelector((state) => state.bookingDataSlice);
  ...
  const handleBook = async () => {
    try {
      if (!singleVehicleDetail || !singleVehicleDetail._id) {
        return toast.error("Handshake failed: Identity unverified.");
      }
      
      if (!pickupDate || !dropoffDate) {
        toast.info("Timeline context missing. Please initialize search from Home.");
      ...
      toast.error("Internal transaction synchronization failure.");
    }
  };
  ...
  if (!singleVehicleDetail) {
    return (
      <div className="flex justify-center items-center h-screen">
        <p>Vehicle details not found. Please return to the vehicle list.</p>
      </div>
    );
  }
            ...
            <div className="lg:col-span-2 lg:row-span-2 lg:row-end-2 bg-slate-50 px-10 min-h-screen  pb-5 pt-10 ">
              ...
              <div className="mt-10 flex flex-col items-center justify-between space-y-4 border-t border-b py-4 sm:flex-row sm:space-y-0">
                <div className="flex items-end">
                  ...
                  <button
                    type="button"
                    className="inline-flex items-center justify-center rounded-2xl border-none bg-slate-900 px-12 py-4 text-center text-xs font-black uppercase tracking-widest text-white shadow-2xl shadow-slate-200 transition-all hover:bg-slate-800 active:scale-95 gap-3"
                    onClick={handleBook}
                  >
                    <GrSecure size={16} className="text-green-400" />
                    Book Experience
                  </button>
              </div>
