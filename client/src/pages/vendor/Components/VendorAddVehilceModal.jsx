...
const VendorAddProductModal = () => {
  ...
  const onSubmit = async (addData) => {
    try {
      ...
      // Collect all images from different fields correctly
      const imageFields = ['image', 'insurance_image', 'rc_book_image', 'polution_image'];
      imageFields.forEach(field => {
        if (addData[field] && addData[field].length > 0) {
          Array.from(addData[field]).forEach(file => {
