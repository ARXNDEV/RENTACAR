...
export const vendorSignup = async (req, res, next) => {
  ...
  try {
    const existingVendor = await User.findOne({ $or: [{ email }, { username }] });
    if (existingVendor) {
...
export const vendorSignin = async (req, res, next) => {
  ...
  try {
    ...
    if (!validVendor) {
      return next(errorHandler(404, "Account not found. Please sign up."));
    }
    if (!validVendor.isVendor) {
      return next(errorHandler(403, "This account is not registered as a vendor."));
    }
    ...
    const { password: hashedPassword, ...rest } = validVendor;
  ...
  try {
    ...
    if (user && user.isVendor) {
     ...
     const userObject = savedUser.toObject();
      ...
      const token = Jwt.sign({ id: newUser._id }, process.env.ACCESS_TOKEN, { expiresIn: '1h' });
