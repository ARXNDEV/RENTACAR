...
export const google = async (req, res, next) => {
  try {
    ...
    if (user) {
      ...
      const accessToken = Jwt.sign({ id: savedUser._id }, process.env.ACCESS_TOKEN, { expiresIn: "1h" });
      const refreshToken = Jwt.sign({ id: savedUser._id }, process.env.REFRESH_TOKEN, { expiresIn: "7d" });
